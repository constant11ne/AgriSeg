...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run repeated hyperparameter sensitivity experiments for top loss families")
    ...
    parser.add_argument("--unconstrained-tversky", action="store_true",
                        help="Search alpha and beta independently instead of beta = 1 - alpha")
    ...
    pd.DataFrame(experiments).drop(columns=["loss_params"]).to_csv(os.path.join(args.out_dir, "experiment_plan.csv"),
                                                                   index=False)
