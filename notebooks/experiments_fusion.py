...
WAVE_C: List[FusionExperiment] = [
    FusionExperiment(
        ...
        nir_init_mode="copy-r",
        wave="C",
        notes="Early-fusion baseline with copy-r initialization",
    ),
    FusionExperiment(
        name="progressive_concat_128",
        fusion_family="mid",
        fusion_method="progressive_concat",
        use_nir=True,
        nir_init_mode="random",
        nir_active_levels=1,
        progressive_level_name="128",
        wave="C",
        notes="Progressive concat fusion: add NIR branch at level 128",
    ),
    FusionExperiment(
        name="progressive_concat_128_64",
        fusion_family="mid",
        fusion_method="progressive_concat",
        use_nir=True,
        nir_init_mode="random",
        nir_active_levels=2,
        progressive_level_name="128_64",
        wave="C",
        notes="Progressive concat fusion: add NIR branch at levels 128 and 64",
    ),
    FusionExperiment(
        name="progressive_concat_128_64_32",
        fusion_family="mid",
        fusion_method="progressive_concat",
        use_nir=True,
        nir_init_mode="random",
        nir_active_levels=3,
        progressive_level_name="128_64_32",
        wave="C",
        notes="Progressive concat fusion: add NIR branch at levels 128, 64 and 32",
    ),
    FusionExperiment(
        name="progressive_concat_128_64_32_16",
        fusion_family="mid",
        fusion_method="progressive_concat",
        use_nir=True,
        nir_init_mode="random",
        nir_active_levels=4,
        progressive_level_name="128_64_32_16",
        wave="C",
        notes="Progressive concat fusion: add NIR branch at levels 128, 64, 32 and 16",
    ),
    FusionExperiment(
        name="progressive_concat_128_64_32_16_8",
        fusion_family="mid",
        fusion_method="progressive_concat",
        use_nir=True,
        nir_init_mode="random",
        nir_active_levels=5,
        progressive_level_name="128_64_32_16_8",
        wave="C",
        notes="Progressive concat fusion: add NIR branch at all levels 128, 64, 32, 16 and 8",
    ),
]
...
def find_best_checkpoint_for_experiment(exp: FusionExperiment, runs_dir: str, model: str, encoder: str) -> Optional[
    str]:
...
def build_command(
    ...
    effective_phase1_freeze_epochs = exp.phase1_freeze_epochs if getattr(exp, "phase1_freeze_epochs",
                                                                         None) is not None else phase1_freeze_epochs
    ...
    if exp.fusion_family in DUAL_STREAM_FAMILIES:
        lite_ckpt = nir_pretrained_lite_wide_ckpt if float(
            getattr(exp, "nir_branch_width", 0.5)) == 1.0 else nir_pretrained_lite_ckpt
        if lite_ckpt:
...
def run_experiments(
    ...
    if late_rgb_ckpt is None:
        late_rgb_ckpt = find_best_checkpoint_for_experiment(
            next((e for e in WAVE_A if e.name == "rgb_only"), None) or FusionExperiment("rgb_only", "rgb",
                                                                                        "early_concat", use_nir=False),
            runs_dir=runs_dir, model=model, encoder=encoder,
        )
    if late_nir_ckpt is None:
        late_nir_ckpt = find_best_checkpoint_for_experiment(
            next((e for e in WAVE_A if e.name == "nir_only"), None) or FusionExperiment("nir_only", "nir",
                                                                                        "early_concat", use_nir=True),
            runs_dir=runs_dir, model=model, encoder=encoder,
        )
    ...
    for idx, exp in enumerate(experiments):
        ...
        exp_dir = summary_payload.get("exp_dir") or os.path.join(runs_dir, expected_run_dir_name(exp, model=model,
                                                                                                 encoder=encoder))
