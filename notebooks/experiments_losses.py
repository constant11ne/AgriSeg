"""First benchmark series for segmentation losses on Agriculture-Vision.

This script compares loss families with fixed default hyperparameters.
Only the loss changes between runs; the rest of the training setup stays fixed.

Saved outputs:
- runs/experiments_losses/summary.csv
- runs/experiments_losses/history_<loss>.csv
"""
...
LOSS_EXPERIMENTS: List[Dict[str, Any]] = [
    ...
    },
    {
        ...
        "params": {"focal_weight": 0.5, "tversky_weight": 0.5, "focal_gamma": 2.0, "tversky_alpha": 0.5, "tversky_beta": 0.5},
    },
]
...
def train_one(cfg: Config, experiment: Dict[str, Any], train_loader, val_loader) -> Dict[str, Any]:
    ...
    for epoch in range(cfg.epochs):
        ...
        train_loss_sum = 0.0
        train_batches = 0
        train_bar = tqdm(enumerate(train_loader), total=cfg.max_train_steps or len(train_loader), desc=f"[{experiment['name']}] Epoch {epoch + 1}/{cfg.epochs} train")
        ...
        val_batches = 0
        metric.reset()
        val_bar = tqdm(enumerate(val_loader), total=cfg.max_val_steps or len(val_loader), desc=f"[{experiment['name']}] Epoch {epoch + 1}/{cfg.epochs} val")
        with torch.no_grad():
