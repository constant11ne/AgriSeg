#!/usr/bin/env python3
"""
notebooks/analyze_fusion_results.py
=====================================
Aggregate and compare all fusion-family experiment results.

This script is the analysis counterpart of experiments_fusion.py.
It reads all ``runs/<exp_name>/metrics.json`` + ``config.json`` files,
builds a summary table, and writes:

    runs/fusion_analysis/
        summary.csv              – one row per experiment, best/final metrics
        per_class.csv            – per-class IoU breakdown
        family_comparison.csv    – grouped by fusion_family
        top10_by_miou.txt        – human-readable top-10 table

Usage
-----
    python -m notebooks.analyze_fusion_results \\
        --runs-dir runs \\
        --out-dir  runs/fusion_analysis \\
        --top 15

To filter to specific experiment names:
    python -m notebooks.analyze_fusion_results \\
        --runs-dir runs \\
        --filter-prefix early mid adapter
"""
...
def main() -> None:
    ...
    # ---- Per-class breakdown for top experiments -----------------------
    ...
    # Also dump a JSON summary for downstream use
