...
from src.utils.sequential_utils import (
    StageSpec,
    candidate_name,
    summarize_repeats,
    decode_params,
    infer_next_primary_refinement,
    choose_shortlist,
    expected_runs_for_stage,
)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--losses",
        nargs="*",
        default=["bce_dice", "bce_tversky", "dice_focal"],
    )
    parser.add_argument("--repeats", type=int, default=3, help="Max repeats for shortlisted configs")
    parser.add_argument("--base-seed", type=int, default=42)

    parser.add_argument("--epochs", type=int, default=5, help="Epochs for final refine stage")
    parser.add_argument("--coarse-epochs", type=int, default=3, help="Epochs for coarse first stage")
    parser.add_argument("--secondary-epochs", type=int, default=4, help="Epochs for second stage")
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Repeats for all configs before shortlisting",
    )
    parser.add_argument(
        "--shortlist-k",
        type=int,
        default=2,
        help="Number of configs to keep after coarse screen",
    )
    parser.add_argument(
        "--final-shortlist-k",
        type=int,
        default=2,
        help="Number of configs to keep in secondary stages before adding repeats",
    )
...
def run_stage(
    ...
    # Phase 1: all candidates get initial_repeats
    ...
    # Phase 2: only shortlist gets extra repeats
    ...
    for loss_name in args.losses:
        ...
        if plan.get("final_refine_primary", False):
            refine_stage = StageSpec(
                ...
                plan["primary_param"],
                [0, 1, 2, 3, 4],  # only length matters for estimate
                epochs=args.epochs,
...
def main() -> None:
    ...
    # BIG SPEEDUP: loaders are created once and reused
