...
def external_rgb_ckpt(args: argparse.Namespace, fold: int) -> Path:
    ...
    return Path(args.rgb_source_runs_dir) / f"fold_{fold}" / f"{args.model}_{args.encoder}_rgb_early_concat_copyr_{LOSS}" / "best.pth"


def external_nir_ckpt(args: argparse.Namespace, fold: int) -> Path:
    if args.nir_checkpoint:
        return Path(args.nir_checkpoint)
    return Path(args.nir_source_runs_dir) / f"fold_{fold}" / f"{args.model}_{args.encoder}_nir_early_concat_copyr_{LOSS}" / "best.pth"
