...
@torch.no_grad()
def evaluate(model: nn.Module, loader: DataLoader, criterion: nn.Module, device: torch.device, epoch: int | None = None, epochs: int | None = None) -> Dict[str, float]:
    model.eval()
    loss_sum = 0.0
    n_batches = 0
    tp = None
    fp = None
    fn = None
    val_bar = tqdm(loader, desc=(f"Val   {epoch}/{epochs}" if epoch is not None and epochs is not None else "Val"), ncols=120, leave=True)
    for x, y in val_bar:
...
def save_checkpoint(path: str, model: nn.Module, optimizer, scheduler, epoch: int, metrics: Dict[str, float], args: argparse.Namespace) -> None:
    state = {
        "epoch": epoch,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Pretrain NIR branches on image-level multilabel anomaly classification")
...
def main() -> None:
    ...
    print(f"[INFO] Device: {device} | AMP: {use_amp} | Train batches: {len(train_loader)} | Val batches: {len(val_loader)}")
