...
class MultiStreamFPN(nn.Module):
    ...
    def __init__(
        ...
        self.nir_align = nn.ModuleList([IBN(ch) if self.use_ibn_align else nn.Identity() for ch in self.nir_feat_channels])
        self.ndvi_align = nn.ModuleList([IBN(ch) if self.use_ibn_align else nn.Identity() for ch in self.ndvi_feat_channels])
