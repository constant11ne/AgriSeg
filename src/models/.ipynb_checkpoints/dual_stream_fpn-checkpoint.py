...
class DualStreamFPN(nn.Module):
    ...
    def __init__(
        ...
        if progressive_level_indices:
            self.progressive_level_indices = [int(x) for x in str(progressive_level_indices).split(',') if str(x).strip() != '']
        else:
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.fusion_method in _BOTTLENECK_METHODS:
            ...
            if self.fusion_method in {"progressive_concat", "progressive_se"} and self.progressive_level_indices is not None:
                active = [idx for idx in self.progressive_level_indices if 0 <= idx < len(rgb_feats) and 0 <= idx < len(nir_feats)]
