...
from src.models.backbone_utils import count_trainable_params, freeze_encoder_full, freeze_encoder_stages, \
    unfreeze_last_n_stages, adapt_first_conv
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--encoder", type=str, default="resnet34",
                        help="Backbone encoder name (e.g. resnet34, efficientnet-b0)")
    ...
    parser.add_argument("--use-ndvi", action="store_true",
                        help="Append fixed NDVI channel computed from NIR and red band")
    parser.add_argument("--train-augment-mode", type=str, default="full", choices=["full", "minimal", "none"],
                        help="Training augmentation policy")
    parser.add_argument("--oversample-rare", action="store_true",
                        help="Use WeightedRandomSampler to oversample rare classes in the training set")
    ...
    parser.add_argument("--class-weights", type=str, default=None,
                        help="JSON list of class weights, e.g. '[1.0, 2.0, ...]'")
    ...
    parser.add_argument("--prefetch-factor", type=int, default=2,
                        help="Prefetch factor for DataLoader (when num_workers>0)")
    ...
    parser.add_argument("--cache-root", type=str, default=None,
                        help="Optional path to compact precomputed image/mask cache")
    parser.add_argument("--cache-mmap", action="store_true",
                        help="Use memory-mapped reads for cache files when possible")
    parser.add_argument("--compile", action="store_true", help="Compile the model using torch.compile if available")
    parser.add_argument("--disable-amp", action="store_true",
                        help="Disable Automatic Mixed Precision (AMP) even when using CUDA")
    ...
    parser.add_argument(
        ...
        choices=["none", "nir_ibn_stem", "nir_ibn_stem_s1", "rgb_nir_ibn_stem", "rgb_nir_ibn_stem_s1",
                 "rgb_ndvi_ibn_features", "rgb_nir_ndvi_ibn_features"],
...
def _load_encoder_compatible(target_module: nn.Module, ckpt_path: str, label: str) -> None:
    ...
    print(
        f"[INFO] Loaded pretrained {label} weights from {ckpt_path} | compatible={len(compatible)} skipped={len(skipped)}")
    if missing:
...
def _load_pretrained_nir_branch(model: nn.Module, fusion_family: Optional[str], ckpt_path: Optional[str],
                                fusion_method: Optional[str] = None) -> None:
    if not ckpt_path:
        return
...
def prepare_experiment_name(args: argparse.Namespace) -> str:
    ...
    if str(getattr(args, "fusion_method", "")).startswith("progressive_concat") or getattr(args, "fusion_method",
                                                                                           None) == "progressive_se":
...
def main() -> None:
    ...
    for epoch in range(args.epochs):
        ...
        train_loss = torch.zeros((), device=args.device if args.device.startswith("cuda") else "cpu",
                                 dtype=torch.float64)
        num_batches = 0
        ...
        for batch_idx, (images, masks) in enumerate(train_bar):
                ...
                with torch.inference_mode():
                    if use_amp:
                        with torch.amp.autocast(device_type=device_type, dtype=amp_dtype, enabled=use_amp):
                            outputs = model(
                                _select_model_input(images, fusion_family, getattr(args, "use_ndvi", False)))
            ...
            if args.log_every_n_steps > 0 and (
                    ((batch_idx + 1) % args.log_every_n_steps == 0) or (batch_idx + 1 == len(train_loader))):
                train_bar.set_postfix(loss=f"{float(loss.detach().cpu()):.3f}",
                                      gnorm=f"{float(gnorm.detach().cpu()):.2f}")
