...
from src.models.gvi import GVIOnlySegModel, RGBGVIStreamFPN, LearnableRatioGVI
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--use-gvi", action="store_true", help="Use learnable GVI branch/module computed from RGB+NIR inside the model")
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        choices=["rgb", "nir", "ndvi", "gvi", "early", "mid", "late", "adapter", "attention"],
        help=(
    ...
    parser.add_argument(
        ...
        default=None,
        choices=[
            "early_concat", "early_rgb_ndvi", "early_rgb_gvi",
            ...
            "progressive_concat_rgb_nir_ndvi", "progressive_concat_rgb_ndvi", "progressive_concat_rgb_gvi", "gvi_ratio",
    ...
    parser.add_argument(
        ...
        choices=["none", "nir_ibn_stem", "nir_ibn_stem_s1", "rgb_nir_ibn_stem", "rgb_nir_ibn_stem_s1", "rgb_ndvi_ibn_features", "rgb_nir_ndvi_ibn_features", "rgb_gvi_ibn_features"],
    ...
    parser.add_argument(
        "--gvi-pretrained-path", type=str, default=None,
        help="Optional checkpoint used to initialize GVI encoder/module from NDVI-only or GVI-only checkpoint.",
    )
    parser.add_argument(
        "--gvi-channels", type=int, default=1,
        help="Number of learnable GVI channels produced by the GVI module.",
    )
    parser.add_argument(
        "--gvi-init-indices", type=str, default="ndvi",
        help="Comma-separated GVI initializers, e.g. 'ndvi,gndvi,ndwi,random'.",
    )
    parser.add_argument(
...
def build_model(
    ...
    *,
    ...
    gvi_channels: int = 1,
    gvi_init_indices: Optional[str] = None,
) -> nn.Module:
    ...
    if fusion_family is not None:
        return _build_model_fusion_family(
            ...
            gvi_channels=gvi_channels,
            gvi_init_indices=gvi_init_indices,
        )
...
class EarlyRGBGVIModel(nn.Module):
    """Early-fusion wrapper: compute learnable GVI from RGB+NIR, then segment RGB+GVI."""
    ...
    def __init__(
        ...
        num_classes: int,
        gvi_channels: int = 1,
        gvi_init_indices: Optional[List[str]] = None,
        first_conv_init: str = "copy-r",
    ) -> None:
        super().__init__()
        if not HAS_SMP:
            raise ImportError("segmentation_models_pytorch is required for EarlyRGBGVIModel.")
        self.gvi_module = LearnableRatioGVI(
            num_channels=gvi_channels,
            init_indices=gvi_init_indices,
        )
        self.seg_model = _build_smp_model(
            model_name,
            encoder_name,
            encoder_weights,
            num_classes,
        ...
        _replace_first_conv(self.seg_model, new_in_channels=3 + int(gvi_channels), init_mode=first_conv_init)
    ...
    @property
    def rgb_encoder(self) -> nn.Module:
        return self.seg_model.encoder

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        if x.shape[1] < 4:
            raise RuntimeError(f"early_rgb_gvi expected RGB+NIR input with 4 channels, got {x.shape[1]}")
        gvi = self.gvi_module(x[:, :4])
        return self.seg_model(torch.cat([x[:, :3], gvi], dim=1))
...
def _build_model_fusion_family(
    ...
    if fusion_family == "gvi":
        gvi_init_list = [x.strip() for x in str(gvi_init_indices or "ndvi").split(",") if x.strip()]
        return GVIOnlySegModel(
            model_name=model_name,
            encoder_name=encoder,
            encoder_weights=encoder_weights,
            num_classes=num_classes,
            gvi_channels=gvi_channels,
            gvi_init_indices=gvi_init_list,
        )

    if fusion_method == "early_rgb_gvi":
        gvi_init_list = [x.strip() for x in str(gvi_init_indices or "ndvi").split(",") if x.strip()]
        return EarlyRGBGVIModel(
            model_name=model_name,
            encoder_name=encoder,
            encoder_weights=encoder_weights,
            num_classes=num_classes,
    ...
    if fusion_method == "progressive_concat_rgb_gvi":
        if not HAS_DUAL_STREAM:
            raise ImportError("segmentation_models_pytorch is required for RGBGVIStreamFPN.")
        gvi_init_list = [x.strip() for x in str(gvi_init_indices or "ndvi").split(",") if x.strip()]
        return RGBGVIStreamFPN(
            num_classes=num_classes,
            encoder_name=encoder,
            encoder_weights=encoder_weights,
            fusion_method=fusion_method,
...
def _load_encoder_compatible(target_module: nn.Module, ckpt_path: str, label: str) -> None:
    ...
    for prefix in ("encoder.", "model.encoder.", "seg_model.encoder.", f"{label.lower()}_encoder.", ""):
...
def _load_module_compatible(target_module: nn.Module, ckpt_path: str, prefix: str, label: str) -> None:
    state = _load_state_dict_flexible(ckpt_path)
    if not any(k.startswith(prefix) for k in state.keys()):
        print(f"[INFO] No {label} module weights with prefix '{prefix}' found in {ckpt_path}; keeping current init")
        return
    cleaned = {k[len(prefix):]: v for k, v in state.items() if k.startswith(prefix)}
    target_state = target_module.state_dict()
    compatible = {k: v for k, v in cleaned.items() if k in target_state and getattr(target_state[k], "shape", None) == getattr(v, "shape", None)}
    target_module.load_state_dict(compatible, strict=False)
    print(f"[INFO] Loaded pretrained {label} module from {ckpt_path} | compatible={len(compatible)}")


def _load_pretrained_gvi_branch(model: nn.Module, fusion_family: Optional[str], ckpt_path: Optional[str]) -> None:
    if not ckpt_path:
        return
    if hasattr(model, "gvi_encoder"):
        _load_encoder_compatible(model.gvi_encoder, ckpt_path, "GVI")
    elif fusion_family == "gvi" and hasattr(model, "encoder"):
        _load_encoder_compatible(model.encoder, ckpt_path, "GVI")
    else:
        print(f"[WARN] --gvi-pretrained-path encoder part ignored for fusion_family={fusion_family}")
    if hasattr(model, "gvi_module"):
        _load_module_compatible(model.gvi_module, ckpt_path, "gvi_module.", "GVI")
...
def prepare_experiment_name(args: argparse.Namespace) -> str:
    ...
    if getattr(args, "use_ndvi", False):
        parts.append("ndvi")
    if getattr(args, "use_gvi", False):
        gvi_channels = int(getattr(args, "gvi_channels", 1))
        parts.append(f"gvi{gvi_channels}" if gvi_channels != 1 else "gvi")
    return "_".join(parts)
...
def main() -> None:
    ...
    model = build_model(
        ...
        use_nir=args.use_nir,
        use_ndvi=getattr(args, "use_ndvi", False),
        use_gvi=getattr(args, "use_gvi", False),
        fusion_family=fusion_family,
        fusion_method=fusion_method,
        freeze_rgb=getattr(args, "freeze_rgb_encoder", "none"),
        ...
        late_rgb_model=late_rgb_model,
        late_nir_model=late_nir_model,
        gvi_channels=getattr(args, "gvi_channels", 1),
        gvi_init_indices=getattr(args, "gvi_init_indices", "ndvi"),
    )
    ...
    _load_pretrained_gvi_branch(model, fusion_family, getattr(args, "gvi_pretrained_path", None))
