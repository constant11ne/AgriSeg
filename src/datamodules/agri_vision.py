...
class BasicTransforms:
    ...
    def __init__(self, img_size: Tuple[int, int] = (512, 512), augment: bool = True, use_nir: bool = False,
                 use_ndvi: bool = False, augment_mode: str = "full") -> None:
        self.img_size = img_size
    ...
    def __call__(self, image: np.ndarray, mask: np.ndarray) -> dict:
        ...
        if _HAS_CV2:
            ...
            mask_resized_hwc = cv2.resize(mask_hwc, (target_w, target_h), interpolation=cv2.INTER_NEAREST).astype(
                np.float32)
        ...
        if img_resized.ndim == 2:
            img_resized = np.stack([img_resized] * (3 + (1 if self.use_nir else 0) + (1 if self.use_ndvi else 0)),
                                   axis=-1)
        n_channels = 3 + (1 if self.use_nir else 0) + (1 if self.use_ndvi else 0)
...
def get_transforms(
    ...
    return BasicTransforms(img_size=img_size, augment=augment, use_nir=use_nir, use_ndvi=use_ndvi,
...
def build_dataloaders(
    ...
    train_transforms = get_transforms(img_size, augment=augment, use_nir=use_nir, use_ndvi=use_ndvi,
                                      augment_mode=augment_mode)
    ...
    train_dataset = AgricultureVisionDataset(
        root=data_root, split="train", transform=train_transforms, img_size=img_size, use_nir=use_nir,
        use_ndvi=use_ndvi,
