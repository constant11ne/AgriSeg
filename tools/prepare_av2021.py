...
def main():
    ...
    ap.add_argument("--fake-masks", action="store_true",
                    help="Create zero masks when labels are missing (e.g., test split)")
    args = ap.parse_args()
    ...
    for split in [s.strip() for s in args.splits.split(",") if s.strip()]:
        ...
        if not src_split.exists():
            print(f"[WARN] split missing: {src_split}, skipping");
            continue
